...
PROJECTS = [
    ...
    },
    {
        ...
        "repo": "PowerElecLLM",
        "category": "AI/ML for circuits & engineering",
        "posture": "Strong RAG and agentic-evaluation project; keep dataset scope explicit",
        "signal": "strong",
        "summary": "SPICE-grounded benchmark, RAG, agentic repair, structured-output and MCP system for evaluating and improving LLMs on power-electronics circuit design.",
        "highlights": [
            "README documents a July 2026 RAG ablation on the test split: GPT-4o base SPICE pass rate 26.7% versus GPT-4o plus RAG at 35.0%, an 8.3-point gain and 31% relative improvement.",
            "Implemented topology-tagged retrieval over design-equation references under knowledge_base and src/rag, with evaluation scripts recording RAG metadata and retrieved chunk IDs.",
            "Built a bounded LangGraph and LangChain design-repair agent that generates Pydantic DesignProposal structured outputs, runs SPICE feedback loops and has offline tests for repair recovery, attempt bounds, unit conversion and two-step generation.",
            "Added an MCP server exposing simulate_converter, search_knowledge_base and get_design_reference tools over FastMCP, with offline tests for simulation, alias normalization, keyword fallback retrieval and topology references.",
            "Repo also contains fine-tuning and multi-model benchmark material; README uses both 500-problem benchmark wording and 650-problem historical wording, so resume claims should keep the scope explicit.",
        ],
        "tech": "Python, ngspice, RAG, LangGraph, LangChain, Pydantic structured outputs, MCP/FastMCP, LLM evaluation and fine-tuning, benchmark design, OpenAI/Gemini/LLaMA/Grok adapters, JSONL, statistical reporting",
        "resume": "Built a SPICE-grounded power-electronics LLM evaluation and agent stack with RAG over design equations, LangGraph repair loops, Pydantic structured outputs and MCP tools; RAG improved GPT-4o SPICE pass rate from 26.7% to 35.0% on the reported test split.",
        "verify": "Use the 26.7% to 35.0% RAG ablation as the strongest metric. Do not frame structured outputs as improving accuracy: the README says the structured-output agent reduced first-shot pass rate versus free-text RAG. Keep the 500 versus 650 benchmark-size distinction explicit.",
        "sources": "README.md, knowledge_base, src/rag, src/agent, src/mcp_server, tests/test_design_agent.py, tests/test_mcp_server.py, benchmark result metadata.",
    },
    {
        "name": "OhmAI",
        "repo": "OhmAI",
        "category": "AI/ML for circuits & engineering",
        "posture": "Strong LLM guardrails and observability project; reconcile old circuit test artifact",
        "signal": "mixed",
        "summary": "FastAPI application that converts natural-language requests into SPICE netlists using Gemini, validates circuits through ngspice, emits Datadog telemetry and now includes trust-and-safety guardrails for LLM-generated simulation input.",
        "highlights": [
            ...
            "Added app/security input_guard and netlist_sanitizer modules: prompt-injection screening, request length and charset validation, fail-closed netlist scanning and blocking for shell/system commands, file includes, source directives, command substitution and path traversal before ngspice execution.",
            "SECURITY.md documents the threat model: LLM output is attacker-influenceable and ngspice is not a sandbox, so the final netlist sanitizer is the authoritative boundary.",
            "test_security.py covers 20 guardrail tests including 9 malicious netlist attack vectors, prompt-injection descriptions, oversized input, unsafe circuit_type strings and safe-comment handling.",
            "README still claims a 39/39 circuit validation suite, while committed test_results.json contains 33 HTTP_ERROR entries with HTTP 422; avoid using circuit pass-rate numbers until the stale/negative suite is reconciled.",
        ],
        "tech": "Python, FastAPI, Gemini/Vertex AI, ngspice, Datadog APM/StatsD/events, input validation, prompt-injection screening, netlist sanitization, Docker, Cloud Run, Chart.js",
        "resume": "Added trust-and-safety guardrails to a natural-language-to-SPICE FastAPI service, screening user prompts before LLM calls and fail-closing unsafe ngspice netlists before simulator execution.",
        "verify": "Use OhmAI for LLM safety, secure code generation, simulator sandboxing and observability roles. Do not use 39/39 circuit-validation pass claims until test_results.json is reconciled; security tests are separate and currently documented as 20 tests.",
        "sources": "README.md security section, SECURITY.md, app/security/input_guard.py, app/security/netlist_sanitizer.py, app/services/spice.py, app/main.py, test_security.py, test_results.json.",
    },
    {
...
DISCREPANCIES = [
    ...
    ("PowerElecLLM", "README alternates between 650 historical wording and 500-problem benchmark wording.", "Keep benchmark-scope wording explicit and tie each metric to the README ablation section or result metadata."),
    ("OhmAI", "README claims 39/39 circuit validation passing; committed test_results has 33 HTTP 422 errors.", "Treat the 20-test security suite separately from circuit validation and avoid circuit pass-rate claims until a dated passing report is committed."),
